...
class RESIT_PRIOR(_BaseLiNGAM):
    ...
    def __init__(self, regressor, prior_knowledge=None,  random_state=None, alpha=0.01):
    ...
    def _estimate_order(self, X, prior_knowledge=None):
        """Determine topological order"""

        S = np.arange(X.shape[1]) # inds of available nodes
        pa = {} # parents of each node
        pi = [] # causal order 

        if prior_knowledge is not None: # currently only meant to inject prior about single sink
            pk = prior_knowledge.copy()
            # get the sinks from the prior knowledge
            prior_sinks = np.unique(np.where(pk==0)[1])
